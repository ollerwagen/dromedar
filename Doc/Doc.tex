...
\lstdefinelanguage{Dromedar}{
	...
	morekeywords = [8]{Str,IO,Util,File,Math,Regex,Sys,Time,List,Turtle},
	morekeywords = [9]{main,of_int,of_flt,print_int,print_flt,print_char,print_bool,print_char,print_str,randint,randflt,readall,sin,cos,tan,e,pi,compile,matches,first_match,all_matches,cmd,fork,clock,time,sleep_for,now,dt,s,ms,us,of_s,of_ms,of_us,map,make,close,fd,bk,lt,rt,penUp,penDn,setPenColor,setPenWidth,showTurtle,hideTurtle,length,push,pop,insert,insert_all,erase,sub},
	comment = [l]{\#},
	%morecomment = [s]{/*}{*/},
	string = [b]',
	morestring = [b]"
...
\newenvironment{stdlib}{\begin{longtable}{p{0.2\linewidth}|p{0.4\linewidth}|p{0.3\linewidth}}}{\end{longtable}}
	...
	\section{Buildup of a Program}
		...
		\subsection{Builtin Types}
			...
			\subsubsection{Arrays}
				...
				The simplest builtin type (after the four primitive types) is the \textit{array} type. Arrays are dynamic. Arrays have the following pseudo-fields and functions (for the array type \code{[t]}):
				
				\begin{stdlib}
					\textbf{Name} & \textbf{Type} & \textbf{Description} \\
					\midrule
					operator \code{[]} & \code{int -> t} & returns the element with the given index and allows the element to be modified \\
					\code{length} & \code{int} & returns the number of elements in the array \\
					\code{push} & \code{t -> [t]} & appends a new element to the array (in-place modification), and returns a \code{this} reference \\
					\code{pop} & \code{void -> [t]} & removes the last element in the array (in-place modification), and returns a \code{this} reference \\
					\code{insert} & \code{(int,t) -> [t]} & inserts a value at the given position \\
					\code{insert_all} & \code{(int,[t]) -> [t]} & inserts an array at the given position \\
					\code{erase} & \code{int -> [t]} & removes the value at the given index \\
					\code{sub} & \code{(int,int) -> [t]} & returns the subarray with from the start (1\textsuperscript{st} argument) with a given number of elements (2\textsuperscript{nd} argument) \\
				\end{stdlib}
			
				Since \code{string} and \code{[char]} are equivalent, these member fields and methods also exist for strings.
		...
		\subsection{Standard Library}
			...
			\subsubsection{\code{Regex}: Regular Expressions}
				...
				\begin{stdlib}
					...
					\code{Regex.compile} & \code{string -> R?} & returns \code{null} if the compilation to a regex automaton fails \\
					\code{Regex.matches} & \code{(R,string) -> bool} & finds whether a partial match in the given string exists \\
					\code{Regex.first_match} & \code{(R,string) -> string?} & returns the first regex match (if it exists, else \code{null}) \\
					\code{Regex.all_matches} & \code{(R,string) -> [string]} & finds all matches
				\end{stdlib}
			...
			\subsubsection{\code{Sys} : System Calls, etc.}
				...
				\begin{stdlib}
					\textbf{Name} & \textbf{Type} & \textbf{Description} \\
					\midrule
					\code{Sys.cmd} & \code{string -> int} & executes a shell command \\
					\code{Sys.fork} & \code{() -> int} & executes the \code{fork()} \textsc{Unix} system call
				\end{stdlib}
			...
			\subsubsection{\code{Time} : Date and Time Utilities}
				...
				\begin{stdlib}
					...
					\code{Time.clock} & \code{() -> int} & the number of clock ticks since the start of the program \\
					\code{Time.time} & \code{() -> int} & returns the \textsc{Unix} time \\
					\code{Time.now} & \code{() -> P} & the time point representing the point when \code{now()} was called \\
					\code{Time.dt} & \code{(P,P) -> D} & calculates the time difference between the first and the second time point \\
					\code{Time.s} & \code{D -> int} & duration in seconds \\
					\code{Time.ms} & \code{D -> int} & duration in milliseconds \\
					\code{Time.us} & \code{D -> int} & duration in microseconds \\
					\code{Time.of_s} & \code{int -> D} & duration from seconds \\
					\code{Time.of_ms} & \code{int -> D} & duration from milliseconds \\
					\code{Time.of_us} & \code{int -> D} & duration from microseconds
				\end{stdlib}
			
			\subsubsection{\code{List} : List Manipulation Functions}
			
				\begin{stdlib}
					\textbf{Name} & \textbf{Type} & \textbf{Description} \\
					\midrule
					\code{List.map} & \code{(<a>-><b>,[<a>]) -> [<b>]} & applies a function on all elements of a list
				\end{stdlib}
			
		\subsection{Educational Library}
		
			Next to the Standard Library, Dromedar also has a library designed for educational purposes -- currently containing a turtle graphics module using \textbf{SFML}. It contains the following functions:
			
			\subsubsubsection{\code{Turtle} : Turtle Graphics Module}
			
				\begin{stdlib}
					\textbf{Name} & \textbf{Type} & \textbf{Description} \\
					\midrule
					\code{make} & \code{() -> void} & creates a new turtle window \\
					\code{close} & \code{() -> void} & waits until the user presses the button to close the window \\
					\code{fd} & \code{flt -> void} & moves the turtle forward \\
					\code{bk} & \code{flt -> void} & moves the turtle backward \\
					\code{rt} & \code{flt -> void} & rotates the turtle clockwise \\
					\code{lt} & \code{flt -> void} & rotates the turtle counterclockwise \\
					\code{penUp} & \code{() -> void} & turtle stops drawing \\
					\code{penDn} & \code{() -> void} & turtle starts drawing \\
					\code{setPenColor} & \code{string -> void} & sets the pen color to the input. If the string is not a color, the program raises an error \\
					\code{setPenWidth} & \code{flt -> void} & sets the width of the pen \\
					\code{showTurtle} & \code{() -> void} & shows turtle sprite and animates its movements \\
					\code{hideTurtle} & \code{() -> void} & hides the turtle and skips through its movements
				\end{stdlib}
...
f(null of [string], [null of string])
		...
		\subsection{Statements}
			...
			\subsubsection{Repeat Statement}
				...
				\begin{align*}
					\drmrule{S\ruleapp{E}\mc{x}::\mc{int}\ \ S,T,\mc{rt},\top\ruleapp{S}\mc{b}\Rightarrow S',R,B}{S,T,\mc{rt},L}{S}{\mc{repeat x b} \Rightarrow S,\bot,\bot}{StmtRepeat}
				\end{align*}
	...
	\section{Formal Grammar}
		...
		\subsection{Parser Grammar}
			...
			\begin{small}\begin{verbatim}
				...
				GlobalStatement  ::= GVDeclaration | GFDeclaration | GTDeclaration
				                     | %Module %Identifier | %Native NatDecl | %Using %Identifier
				...
				GTDeclaration    ::= %Type %Identifier { (%Identifier {%Comma %Identifier} %Colon Type) |
				                       GFDeclaration }
				
				NatDecl          ::= %Type %Identifier | Type %Identifier
				...
				Statement        ::= VDeclaration | AssignStmt
				                     | IfStmt | NullCastStmt
				                     | WhileStmt | DoWhileStmt | ForStmt | RepeatStmt
				                     | ExprStmt
				                     | %Break | %Continue
				                     | ReturnStmt
				...
				ForStmt          ::= %For %Id %Assign Expr RangeSpecifier Expr Block
				...
				Expression       ::= %Null %Of RefType
				                     | %LBrack %RBrack %Of Type
				                     | %QuestionMark Expression %Arrow Expression %Colon Expression
 				                     | %Assert Expression
				                     | ExprPrec30
				
				ExprPrec20       ::= ExprPrec30 [ (%Equal | %NotEqual | %Greater | %Less | %GreaterEq
				...
				BaseExpression   ::= %LParen Expression %RParen
				...
				Application      ::= %LParen [ FArg { %Comma FArg } ] %RParen
